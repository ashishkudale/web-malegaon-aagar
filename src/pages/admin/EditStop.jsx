...
const EditStop = () => {
  ...
  const [fetchingStop, setFetchingStop] = useState(true);
  ...
  const loadStop = async () => {
    try {
      ...
      setFormData({
        stopNameEnglish: stop.stopNameEnglish || '',
      ...
      alert('Error loading stop');
      console.error(error);
      navigate('/admin/stops');
    } finally {
      setFetchingStop(false);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await stopService.updateStop(stopId, stopData);
  ...
  if (fetchingStop) {
    return <div className="admin-page"><div>Loading stop...</div></div>;
  }

  return (
    <div className="admin-page">
      <h1>Edit Stop</h1>
      <p className="subtitle">स्टॉप संपादित करा</p>
